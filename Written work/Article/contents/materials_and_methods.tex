...
The TF implementation of the VGGNet is heavily inspired by the work of \todo{insert name of author}\cite{git_vgg16}. 
...
There are several approaches to feed the LSTM cell. The chosen approach has been inspired by the work of \todo{insert author} \cite{git_rnn_cnn_1,43455}, where the LSTM cell is acting on each of the generated feature maps.
...
The implementation of LSTM cell is heavily inspired by the work of \todo{insert aotuehr names}\cite{git_lstm}. This implementation takes the input tensor from the final max pooling layer in the VGGNet and returns the state tensor. Both are 4D-tensors (batch x height x width x channels).
