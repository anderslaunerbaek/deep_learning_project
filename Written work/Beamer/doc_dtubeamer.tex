...
\title[Anders Launer Baek]{Deep Convolutional and Recurrent Neural Networks for Interpretable Analysis of EEG Sleep Stage Scoring}
...
\end{tabular}
\caption{The distribution of the labels for all 20 subjects.}
...
\frame{
	...
	\begin{itemize}
		\item<1-1> Successfully re-produced \cite{main_ar} in TF which was one of the objectives, despite using few different approaches.
		\item<1-1> Added a LSTM cell to the of the VGGNet 16 network.  The of the RNN does out-perform the baseline on average (table \ref{tab_res_2}), despite the RNN does archive better classification sensitivity in the following sleeping stages W and R.
		\item<2-2> Improvements in this projects can be archived by merging sleeping stage N3 and N4, then follow the newest regulations.
		\item<2-2> Study the effect of stacking multiple LSTM cells and applied the LSTM cells from layers with a lower-level feature representation higher and spectral variance.
	\end{itemize}
